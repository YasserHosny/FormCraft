...
Phase 1 builds only what is specified. No SSO. No OCR. No AI chat assistant. No layout AI. **No general-purpose bulk automation engine.** No real-time collaboration. No version diffing UI. Features are added only when specified in an approved spec. Premature abstraction is avoided — extract patterns only after the third occurrence.

> **Approved Exception (F31)**: Bulk governance actions (archive, reassign, category change) are permitted when explicitly specified in an approved governance feature spec. These actions are constrained to admin-selected templates and must include audit logging, confirmation warnings, and status-aware guardrails.
