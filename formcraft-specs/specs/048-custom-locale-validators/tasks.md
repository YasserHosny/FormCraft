...
- [x] Write PostgreSQL migration to create `custom_validators` table
- [x] Add columns: id, org_id, name, description, regex_pattern, error_message_ar, error_message_en, created_at, updated_at, created_by, updated_by, deleted_at
- [x] Add UNIQUE constraint on (org_id, name)
- [x] Add CHECK constraints for non-empty fields
- [x] Create indexes on (org_id, deleted_at) for fast org queries
- [x] Create indexes on (org_id, name) for dropdown population

### Task 1.2: Modify elements Table
- [x] Add `custom_validators_ids UUID[]` column with default empty array
- [x] Add migration to handle existing elements (set to empty array)

### Task 1.3: Implement RLS Policies
- [x] Create RLS policy: users can read custom_validators only for their org
- [x] Create RLS policy: users can write (insert/update/delete) only if org_id matches their profile org_id
...
- [x] **1.4.I** `GET /admin/validators` — paginated, `ILIKE` search across `name || ' ' || COALESCE(description, '')` (FR-8)
- [x] **1.4.I** `POST /admin/validators` — create with validation (required bilingual messages, regex compiled in sandbox + nested-quantifier check + 100ms probe timeout per FR-9)
- [x] **1.4.I** `GET /admin/validators/:id`
- [x] **1.4.I** `PUT /admin/validators/:id` — same FR-9 regex checks on changed pattern; emit `VALIDATOR_UPDATED` with changed-field diff
- [x] **1.4.I** `DELETE /admin/validators/:id` — soft delete; cascade behavior: existing element references remain but validator is skipped at runtime
- [x] **1.4.I** Rate limiting: 30 req/min/org on POST and PUT (FR-1)
- [x] **1.4.I** Enforce hard cap: reject POST if `COUNT(* WHERE org_id=? AND deleted_at IS NULL) >= 500`
...
- [x] **1.4b.I** Implement compile-time pre-check (length ≤ 500, no `(...+)+` / `(...*)*` chains via AST scan)
- [x] **1.4b.I** Implement compile-time probe runner with 100ms hard timeout (Python `signal.SIGALRM` or subprocess)
...
- [x] **1.5.I** `GET /api/validators/org` — return active validators sorted `ORDER BY name ASC` (US-3)
- [x] **1.5.I** In-process cache keyed by `org_id`, TTL 60s, explicit invalidation hook called from POST/PUT/DELETE on `/admin/validators`
...
- [x] **1.6.I** `GET /admin/validators/:id/templates` — derived join: `templates → pages → elements WHERE :validator_id = ANY(elements.custom_validators_ids)`
- [x] **1.6.I** `last_submission_at` computed via correlated subquery `MAX(submissions.created_at) WHERE submissions.template_id = templates.id` (FR-6, U1 resolved)
- [x] **1.6.I** Response fields: `template_id`, `template_name`, `template_status`, `last_submission_at` (nullable)

### Task 1.7: Add Audit Logging
- [x] Create audit log entries for VALIDATOR_CREATED event
- [x] Create audit log entries for VALIDATOR_UPDATED event (with before/after values)
- [x] Create audit log entries for VALIDATOR_DELETED event
...
- [x] Create `/admin/validators` route in Admin Console
- [x] Design page layout: list + detail panel
- [x] Add toolbar with search, filter, create button

### Task 2.2: Implement Validators List Component
- [x] Display table/list of org's custom validators
- [x] Columns: name, description, pattern (truncated), created_by, updated_at
- [x] Pagination (20 per page)
- [x] Search by name and description
- [ ] Sort by name, created_at, updated_at

### Task 2.3: Implement Create Validator Modal
- [x] Form fields: name, description, regex_pattern, error_message_ar, error_message_en
- [x] Regex pattern validator: test pattern before save
- [x] Bilingual input: side-by-side AR/EN fields
- [x] Submit button creates validator via POST endpoint
- [x] Show success/error toast

### Task 2.4: Implement Edit Validator Modal
- [x] Load validator details from API
- [x] Pre-populate all fields
- [x] Regex pattern validator (test before save)
- [x] Submit updates validator via PUT endpoint
- [x] Show warning: "Changes will apply to all templates using this validator"

### Task 2.5: Implement Delete Confirmation Dialog
- [x] Show validator name and count of templates using it
- [x] Confirmation text: "This will remove the validator, but existing references will remain (showing the old rules)"
- [x] Delete via DELETE endpoint (soft delete)

### Task 2.6: Implement Usage Tab
- [x] Show list of templates using this validator
- [x] Display: template name, last used date, edit link
...
- [x] Add "Custom Validators" section to validation rules panel
- [x] Position below built-in validation rules

### Task 3.2: Implement Custom Validators Dropdown
- [x] Fetch org's validators via GET /api/validators/org on panel open
- [x] Display: validator name, description in dropdown
- [x] Support search/filter by name
- [x] Handle loading state and empty state

### Task 3.3: Add Validator Tags/Chips UI
- [x] Selected validators appear as removable chips below dropdown
- [x] Clicking X removes validator from element
- [x] Visual indication of multiple validators applied

### Task 3.4: Persist Validator Selections
- [x] Save selected custom_validators_ids array to element.custom_validators_ids
- [x] Ensure array is properly serialized/deserialized in element model
- [x] Handle undo/redo for validator additions/removals
...
- [x] Hover tooltip shows full validator description and regex pattern
- [x] Helps designer understand what the validator checks
...
- [x] **4.1.I** Modify form-fill validator pipeline to: deterministic_chain → custom_chain (per `element.custom_validators_ids` in order)
- [x] **4.1.I** Fetch validator definitions via the cached `GET /api/validators/org` data loaded at form open; refresh cache on page navigation and at submit (FR-7 in-flight semantics)
- [x] **4.1.I** Return structured error: `{validator_id, error_message_ar, error_message_en}`
...
- [x] **4.2.I** Implement selector function `pickErrorMessage(validator, context) → string` honoring the chain
- [x] **4.2.I** Show error inline next to field; use existing built-in validator UI component (no duplicate styling)

### Task 4.3: Add Real-Time Validation UI
- [x] As operator types, check against custom validators in real-time
- [x] Show error immediately when regex matches failure condition
- [x] Hide error when field becomes valid

### Task 4.4: Update Form Submission Handler
- [x] Before submit, validate all custom validators
- [x] Prevent submission if any custom validator fails
- [x] Show all validation errors together
